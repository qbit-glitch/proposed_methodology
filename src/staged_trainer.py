...
class StagedTrainer:
    ...
    def __init__(
        ...
        if self.is_distributed and use_distributed:
            self.model = wrap_model_ddp(self.model, self.device)
        
        # Mixed precision trainer (only on CUDA)
        use_amp = use_mixed_precision and self.device.type == 'cuda'
        self.amp_trainer = MixedPrecisionTrainer(enabled=use_amp, device=self.device)
        ...
        if is_main_process():
            ...
            if self.is_distributed:
                print(f"   Distributed: {torch.distributed.get_world_size()} GPUs")
            if use_amp:
    ...
    def _pretrain_task(
        ...
        """
        Generic task-specific pretraining with checkpoint resume support.
        
        Freezes all components except the target decoder.
        Supports resuming from saved checkpoint with optimizer state.
        """
        print(f"\n{'='*60}")
        ...
        checkpoint_path = os.path.join(self.config.stage2_checkpoint_dir, f"{task_name}_best.pt")
        start_epoch = 0
        best_loss = float('inf')
        ...
        if os.path.exists(checkpoint_path):
            ...
            # Check if this checkpoint has the full training state
            if 'optimizer_state_dict' in checkpoint and 'current_epoch' in checkpoint:
                optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
                start_epoch = checkpoint['current_epoch'] + 1  # Resume from next epoch
                best_loss = checkpoint.get('best_loss', checkpoint.get('loss', float('inf')))
                history = checkpoint.get('history', {'train_loss': [], 'val_loss': []})
                
                if start_epoch >= epochs:
                    print(f"  ✓ {task_name.upper()} already completed ({start_epoch}/{epochs} epochs)")
                    return {'best_loss': best_loss, 'history': history, 'skipped': True}
                
                print(f"  📂 Resuming from epoch {start_epoch + 1}/{epochs} (best_loss: {best_loss:.4f})")
            else:
                print(f"  📂 Found checkpoint but no training state, starting fresh")
        ...
        for epoch in range(start_epoch, epochs):
            ...
            # Determine current loss for comparison
            current_loss = val_loss if val_loss > 0 else train_loss
            
            # Save checkpoint with full training state (always save latest for resume)
    ...
    def _save_task_checkpoint(
        ...
        """Save task-specific checkpoint with full training state for resume."""
        checkpoint = {
            'task_name': task_name,
            'current_epoch': epoch,
            ...
            'completed': (epoch + 1) >= epochs_total,
        }
        torch.save(checkpoint, path)
        if (epoch + 1) >= epochs_total or best_loss == checkpoint['best_loss']:
