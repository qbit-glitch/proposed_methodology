...
@dataclass
class OptimizedModelConfig(ModelConfig):
    """
    Configuration for Optimized Unified Multi-Task Transformer.
    
    Extends ModelConfig with optimization flags:
    - Deformable Attention (OPT-1.1): Adaptive receptive fields, 2-3x faster
    - Hierarchical Decoder (OPT-4.1): Shared backbone + task-specific heads
    - Query Refinement (OPT-1.3): Two-stage query selection
    - Cross-Task Consistency (OPT-4.3): Consistency losses between tasks
    """
    
    # ===== BiFPN (Bidirectional FPN) =====
    use_bifpn: bool = True  # Use BiFPN instead of standard FPN
    
    # ===== Deformable Attention (OPT-1.1) =====
    use_deformable_attention: bool = True
    num_deformable_points: int = 4  # Sampling points per head
    num_feature_levels: int = 3  # Multi-scale levels (P3, P4, P5)
    
    # ===== Hierarchical Decoder (OPT-4.1) =====
    use_hierarchical_decoder: bool = True
    num_shared_decoder_layers: int = 4  # Shared backbone layers
    num_task_specific_layers: int = 2  # Task-specific refinement layers
    
    # ===== Query Refinement (OPT-1.3) =====
    use_query_refinement: bool = True
    num_coarse_decoder_layers: int = 2  # Lightweight coarse decoder
    query_selection_topk: int = 50  # Top-K confident queries to keep
    
    # ===== Cross-Task Consistency (OPT-4.3) =====
    use_cross_task_consistency: bool = True
    consistency_det_seg_weight: float = 0.1  # Detection-Segmentation
    consistency_plate_det_weight: float = 0.1  # Plate-Detection
    consistency_ocr_plate_weight: float = 0.1  # OCR-Plate
    
    # ===== Temporal Tracking Memory (OPT-1.4) =====
    use_temporal_memory: bool = False  # GRU-based feature bank
    temporal_memory_size: int = 10  # Frames to remember
    
    def __post_init__(self):
        """Validate and adjust configuration based on optimizations."""
        # When using hierarchical decoder, adjust layer counts
        if self.use_hierarchical_decoder:
            total_layers = self.num_shared_decoder_layers + self.num_task_specific_layers
            if total_layers != self.num_decoder_layers:
                print(f"  ⚠️ Adjusting decoder layers: {self.num_decoder_layers} → {total_layers}")
                self.num_decoder_layers = total_layers
